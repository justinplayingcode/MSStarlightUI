...
import Api from "../../../api";
import { MappingTypeAppointmentSchedule, accountRole, toastType } from "../../../model/enum";
import { basicKeyValueRender } from "../../../utils/utils";
import { Convert } from "../../../utils";
import { TestList } from "../../../model/doctorModel";
import UniformSection from "../../common/Section";
...
function DetailsCureHistory() {
  ...
  const renderTestResult = (): JSX.Element => {
    if(currentState?.testResult?.length > 0) {
      let element = currentState?.testResult.map((e: any, i: any) => {
        return (
  ...
  const renderSummary = (): JSX.Element => {
    return <>
        {currentState?.diseases.map((e: any, i: any) => {
  ...
  const renderMedication = (): JSX.Element => {
    return <>
      {currentState?.medication.map((e: any, i: any) => {
